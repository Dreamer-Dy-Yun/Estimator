...
describe('mockAdminGoogleSheetApi', () => {
  ...
  it('stores google sheet configs in one company scope without exposing raw service account keys', async () => {
    ...
    const configs = await mockAdminGoogleSheetApi.getAdminGoogleSheetConfigs({ companyUuid: MOCK_HANA_COMPANY_UUID })
  ...
  it('filters google sheet configs by company scope', async () => {
    await mockAuthApi.login({ loginId: 'mock-admin', password: 'admin' })

    const hanaConfigs = await mockAdminGoogleSheetApi.getAdminGoogleSheetConfigs({ companyUuid: MOCK_HANA_COMPANY_UUID })
    const t1Configs = await mockAdminGoogleSheetApi.getAdminGoogleSheetConfigs({ companyUuid: MOCK_T1_COMPANY_UUID })
    const allConfigs = await mockAdminGoogleSheetApi.getAdminGoogleSheetConfigs({ companyUuid: ALL_COMPANY_UUID })

    expect(hanaConfigs.every((config) => config.companyUuid === MOCK_HANA_COMPANY_UUID)).toBe(true)
    expect(t1Configs.every((config) => config.companyUuid === MOCK_T1_COMPANY_UUID)).toBe(true)
    expect(allConfigs.length).toBeGreaterThanOrEqual(hanaConfigs.length + t1Configs.length)
  })

  it('rejects google sheet mutations without a concrete company scope', async () => {
    await mockAuthApi.login({ loginId: 'mock-admin', password: 'admin' })
  ...
  it('deletes google sheet configs by uuid and company scope', async () => {
    await mockAuthApi.login({ loginId: 'mock-admin', password: 'admin' })
    ...
    const configs = await mockAdminGoogleSheetApi.getAdminGoogleSheetConfigs({ companyUuid: MOCK_T1_COMPANY_UUID })

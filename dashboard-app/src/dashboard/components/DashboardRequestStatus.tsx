...
export function DashboardRequestStatus({ items }: DashboardRequestStatusProps) {
  ...
  return (
    <section className={styles.statusBar} aria-live="polite" aria-label="분석 데이터 요청 상태">
      <span className={`${styles.badge} ${tone}`}>{status}</span>
      <span className={styles.message}>{message}</span>
      <span className={styles.updatedAt}>마지막 갱신 {formatUpdatedAt(lastUpdatedAt)}</span>
    </section>
  )
}

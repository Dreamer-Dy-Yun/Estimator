...
export function useCandidateStashItemActions({
  ...
  const currentScopeRef = useRef<MutationScopeSnapshot>({ stashUuid, companyUuid })
  ...
  useEffect(() => {
    currentScopeRef.current = { stashUuid, companyUuid }
  }, [companyUuid, stashUuid])
  ...
  const createMutationScopeSnapshot = useCallback((): MutationScopeSnapshot => ({
    stashUuid,
    companyUuid,
  }), [companyUuid, stashUuid])

  const isCurrentMutationScope = useCallback((snapshot: MutationScopeSnapshot) => (
    currentScopeRef.current.stashUuid === snapshot.stashUuid
      && currentScopeRef.current.companyUuid === snapshot.companyUuid
  ), [])

  const canReflectMutationResult = useCallback((snapshot: MutationScopeSnapshot) => (
    mountedRef.current && isCurrentMutationScope(snapshot)
  ), [isCurrentMutationScope])

  const refreshAfterMutation = useCallback(async (failureMessage: string, snapshot: MutationScopeSnapshot) => {
    if (!canReflectMutationResult(snapshot)) return false
    try {
      await refreshStashes()
      if (!canReflectMutationResult(snapshot)) return false
      return true
    } catch {
      if (canReflectMutationResult(snapshot)) showToast(failureMessage, { variant: 'error' })
      return false
    }
  }, [canReflectMutationResult, refreshStashes, showToast])

  const confirmDeleteItem = useCallback(async () => {
    if (!itemDeleteTarget) return
    const mutationScopeSnapshot = createMutationScopeSnapshot()
    ...
    try {
      try {
        ...
        if (canReflectMutationResult(mutationScopeSnapshot)) showToast(getApiErrorDisplayMessage(err, '후보를 삭제하지 못했습니다.'), { variant: 'error' })
        throw err
      }
      if (!canReflectMutationResult(mutationScopeSnapshot)) return
      if (openedItemUuid === itemDeleteTarget.uuid) closeDrawer()
      onItemsDeleted?.([itemDeleteTarget.uuid])
      const refreshed = await refreshAfterMutation('후보를 삭제했지만 목록을 새로고침하지 못했습니다.', mutationScopeSnapshot)
      if (refreshed) showToast('후보를 삭제했습니다.')
    } finally {
      if (mountedRef.current) setItemDeleteBusy(false)
    }
  }, [canReflectMutationResult, closeDrawer, createMutationScopeSnapshot, itemDeleteTarget, onItemsDeleted, openedItemUuid, refreshAfterMutation, requireCompanyUuid, showToast])
  ...
  const confirmDeleteItems = useCallback(async (itemUuids: string[]) => {
    ...
    try {
      try {
        ...
        if (canReflectMutationResult(mutationScopeSnapshot)) showToast(getApiErrorDisplayMessage(err, '선택 후보를 삭제하지 못했습니다.'), { variant: 'error' })
        throw err
      }
      if (!canReflectMutationResult(mutationScopeSnapshot)) return
      if (openedItemUuid && uniqueUuids.includes(openedItemUuid)) closeDrawer()
      onItemsDeleted?.(uniqueUuids)
      const refreshed = await refreshAfterMutation('선택 후보를 삭제했지만 목록을 새로고침하지 못했습니다.', mutationScopeSnapshot)
      if (refreshed) showToast('선택한 후보를 삭제했습니다.')
    } finally {
      if (mountedRef.current) setBulkDeleteBusy(false)
    }
  }, [canReflectMutationResult, closeDrawer, createMutationScopeSnapshot, onItemsDeleted, openedItemUuid, refreshAfterMutation, requireCompanyUuid, showToast, stashUuid])
  ...
  const confirmUnconfirmItems = useCallback(async (itemUuids: string[]) => {
    ...
    try {
      ...
      try {
        ...
        if (canReflectMutationResult(mutationScopeSnapshot)) showToast(getApiErrorDisplayMessage(err, '선택 후보 상세 확정을 해제하지 못했습니다.'), { variant: 'error' })
        throw err
      }
      if (!canReflectMutationResult(mutationScopeSnapshot)) return
      ...
      if (updatedItems.length) {
        ...
        const refreshed = await refreshAfterMutation('상세 확정 해제는 반영됐지만 목록을 새로고침하지 못했습니다.', mutationScopeSnapshot)
  ...
  }, [canReflectMutationResult, closeDrawer, createMutationScopeSnapshot, onItemsUnconfirmed, openedItemUuid, refreshAfterMutation, requireCompanyUuid, showToast])

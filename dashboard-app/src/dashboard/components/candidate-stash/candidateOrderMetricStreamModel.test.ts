...
describe('candidateOrderMetricStreamModel', () : void => {
  it('normalizes candidate item UUIDs for stable request signatures', () : void => {
  ...
  it('settles a stream when every requested item has emitted a result', () : void => {
    const pending: Set<string> = createPendingMetricItemUuidSet(['item-2', 'item-1', 'item-2'])

...
import { getInboundSplitDetailSectionKeys } from './inboundSplitScheduleDetailRows'
...
export function InboundSplitScheduleDialogV2({
  open,
  ...
  const [expandedSectionKeys, setExpandedSectionKeys]: [ReadonlySet<string>, React.Dispatch<React.SetStateAction<ReadonlySet<string>>>] = useState<ReadonlySet<string>>((): ReadonlySet<string> => new Set<string>())
  ...
  const dateWarningMessage: string = draftError == null
    ...
    : ''
  const detailSectionKeys: string[] = useMemo((): string[] => (
    getInboundSplitDetailSectionKeys(draft.rows, nextOrderInboundDueDate)
  ), [draft.rows, nextOrderInboundDueDate])
  const allDetailSectionsExpanded: boolean = detailSectionKeys.length > 0 && detailSectionKeys.every((sectionKey: string): boolean => expandedSectionKeys.has(sectionKey))
  const canToggleAllDetailSections: boolean = inboundSplitSource != null && detailSectionKeys.length > 0
  const expandAllDetailSections: () => void = useCallback((): void => {
    setExpandedSectionKeys(new Set<string>(detailSectionKeys))
  }, [detailSectionKeys])
  const collapseAllDetailSections: () => void = useCallback((): void => {
    setExpandedSectionKeys(new Set<string>())
  }, [])
  const toggleAllDetailSections: () => void = useCallback((): void => {
    if (allDetailSectionsExpanded) {
      collapseAllDetailSections()
      return
    }
    expandAllDetailSections()
  }, [allDetailSectionsExpanded, collapseAllDetailSections, expandAllDetailSections])
  const toggleDetailSection: (sectionKey: string) => void = useCallback((sectionKey: string): void => {
    setExpandedSectionKeys((currentKeys: ReadonlySet<string>): ReadonlySet<string> => {
      const nextKeys: Set<string> = new Set<string>(currentKeys)
  ...
  return (
    <div className={styles.inboundSplitDialogBackdrop} role="presentation" onClick={onClose}>
      <section
        ref={panelRef}
        className={`${styles.inboundSplitDialog} ${styles.inboundSplitDialogV2}`}
        role="dialog"
        aria-modal="true"
        ...
        <div className={styles.inboundSplitDialogToolbar}>
          ...
          <div className={styles.inboundSplitToolbarActions}>
            <button
              ...
              aria-pressed={allDetailSectionsExpanded}
              onClick={toggleAllDetailSections}
              disabled={!canToggleAllDetailSections}
            >
              {allDetailSectionsExpanded ? KO.btnInboundSplitCollapseAll : KO.btnInboundSplitExpandAll}
            </button>
            <button
              type="button"
              className={`${styles.btn} ${styles.btnSecondary} ${styles.inboundSplitResetConfirmButton}`}

...
export function CandidateStashEditDialog({
  ...
  return (
    <div className={confirmStyles.backdrop} onClick={() : false | void => !editBusy && onClose()}>
      <div className={confirmStyles.panel} onClick={(event: React.MouseEvent<HTMLDivElement, MouseEvent>) : void => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="stash-edit-dialog-title" aria-describedby="stash-edit-dialog-description">
        ...
        <div className={pageStyles.confirmModalForm}>
          ...
          <div className={pageStyles.confirmModalField}>
            ...
            <input id="stash-edit-name" type="text" className={pageStyles.confirmModalInput} value={editName} onChange={(event: React.ChangeEvent<HTMLInputElement, HTMLInputElement>) : void => onNameChange(event.target.value)} disabled={editBusy} autoComplete="off" />
          </div>
          <div className={pageStyles.confirmModalField}>
            <label className={pageStyles.confirmModalLabel} htmlFor="stash-edit-note">비고</label>
            <textarea id="stash-edit-note" className={`${pageStyles.confirmModalInput} ${pageStyles.confirmModalTextarea}`} value={editNote} onChange={(event: React.ChangeEvent<HTMLTextAreaElement, HTMLTextAreaElement>) : void => onNoteChange(event.target.value)} disabled={editBusy} rows={3} />
          </div>
        </div>
        <div className={confirmStyles.actions}>
          ...
          <button type="button" className={`${confirmStyles.button} ${confirmStyles.primaryButton}`} disabled={editBusy || !editName.trim()} onClick={() : undefined => void onSave()}>

...
test('@smoke @candidate 오더 후보군 상세에서 조회 카드, 작업 카드, 추천 진입을 확인할 수 있다', async ({ page }: PlaywrightTestArgs & PlaywrightTestOptions & PlaywrightWorkerArgs & PlaywrightWorkerOptions) : Promise<void> => {
  ...
  await expect(detailDialog.getByText('로딩중...')).toHaveCount(0, { timeout: 10000 })

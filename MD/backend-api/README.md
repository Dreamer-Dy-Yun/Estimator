...
| 3 | [order-snapshot-backend-contract.md](./order-snapshot-backend-contract.md) | 후보 item `confirmedOrderSnapshot`에 저장되는 `OrderSnapshotDocument` v5 계약. |
...
| `dashboard-app/src/snapshot/orderSnapshotTypes.ts` | `OrderSnapshotDocument` v5 type source |
...
- `getSecondaryInboundSplitSource`는 split count/date/result row를 모르는 source-only API다. `dateEnd`는 exclusive다.
- 후보 item `confirmedOrderSnapshot`는 `OrderSnapshotDocument | null`이고 현재 schema version은 `5`다.

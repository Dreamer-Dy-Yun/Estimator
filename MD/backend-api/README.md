...
| 3 | [order-snapshot-backend-contract.md](./order-snapshot-backend-contract.md) | 후보 item `confirmedOrderSnapshot`에 저장되는 `OrderSnapshotDocument` v8 계약. |
...
- `getSecondaryStockOrderCalc` returns `inboundSplitSource` as the shared planning source. Split count/date/result rows and `ignoreExistingOrderInbound` are not request fields.
- 후보 item `confirmedOrderSnapshot`는 `OrderSnapshotDocument | null`이고 현재 schema version은 `8`다.

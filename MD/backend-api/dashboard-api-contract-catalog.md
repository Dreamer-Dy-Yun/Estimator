...
| `sizeRows` | `ProductSecondarySizeRow[]` | 2차 상세 API의 live 사이즈 행. 스냅샷 저장 필드 `drawer2.sizeOrders`와 구분한다. |
...
- Current persisted snapshot document name is `OrderSnapshotDocumentV2`, and the runtime `schemaVersion` is `2`.
...
오더 계산 요청 입력값이다. current parser(`parseOrderSnapshot`)는 `drawer2.stockOrderRequest` 객체와 아래 필드를 검증한다. 화면에 없는 안전재고 모드, 수동 안전재고, 서비스 레벨 값은 저장하지 않는다.
...
- 스냅샷 기준 보기에서 금번/차기 입고 예정일과 리드타임을 복원한다.
- 계산 결과는 `drawer2.stockOrderResult`와 사이즈별 확정값에 따로 저장한다.

### 11.7 `drawer2.stockOrderResult`

오더 계산 API 응답을 저장한 결과 객체다. 계산 API 응답이 없거나 아직 계산 전이면 생략한다.
...
### 11.10 `drawer2.stockOrderResult.display`
...
- 배열 순서는 `sizeOrders`의 `size` 순서와 일치해야 한다.
...
### 11.13 `drawer2.aiComment.prompt`, `drawer2.aiComment.answer`

| 필드 | 타입 | 설명 |
|---|---|---|
| `prompt` | string | AI 코멘트 생성에 사용한 prompt |
| `answer` | string | AI 응답 본문 |

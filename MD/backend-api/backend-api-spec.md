...
- `OrderSnapshotDocumentV1` is deprecated terminology and should appear only in legacy compatibility notes.
...
- New compact drawer2 snapshots use `drawer2.competitorSalesBasis`; `drawer2.secondary` is accepted only as legacy parser input and must not be emitted by new payloads.

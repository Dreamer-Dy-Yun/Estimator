...
- 목 구현은 정적 사용자 seed로 `loginId/password`를 확인한 뒤 `sessionStorage`에 세션만 저장합니다. 사용자 목록/후보군 목록의 실제 변경은 백엔드 DB가 소유해야 하며, 프론트 mock은 mutation 응답 흐름만 모사합니다. 실제 백엔드에서는 가능하면 **HttpOnly cookie 기반 세션**을 권장합니다.

...
| 2차 상세 | `GET /products/:skuGroupKey/secondary` | query | 생략 시 전체 회사 기준 |
...
| 2차 재고·발주 계산 | `POST /products/:skuGroupKey/secondary/stock-order-calc` | payload | 생략 시 전체 회사 기준 |
| 후보군 목록/조회 | `GET /candidate-stashes`, `GET /candidate-stashes/:stashUuid/items`, `GET /candidate-stashes/:stashUuid/recommendations` | query | UI에서 호출 차단 |
| 후보군 mutation | 후보군 생성/수정/삭제/복제, item 추가/삭제/수정 | payload 또는 query adapter 계약 | UI에서 호출 차단 |
| 후보군 SSE/job | 오더 지표 SSE, 후보군 LLM 코멘트 job/SSE, 상세 일괄확정 job/SSE | query 또는 start payload | UI에서 호출 차단 |

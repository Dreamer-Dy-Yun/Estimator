...
> endpoint별 payload/response 필드 설명과 `OrderSnapshotDocumentV2` JSON 구조는 위 문서를 백엔드 구현 기준으로 함께 확인한다.
> 스냅샷 저장/검증/응답 책임은 [`order-snapshot-backend-contract.md`](./order-snapshot-backend-contract.md)를 기준으로 하고, LLM 입력에 사용할 필드 설명은 [`order-snapshot-llm-field-guide.md`](./order-snapshot-llm-field-guide.md)를 기준으로 한다.
...
| `appendCandidateItem(payload)` | POST | `/candidate-stashes/:stashUuid/items` body `{ companyUuid, skuGroupKey, details, isLatestLlmComment }`, 세션 소유자 기준. 성공 응답은 본문 없음(`204 No Content` 권장)이며 프론트 계약은 `Promise<void>` |
...
| `competitorRatioBySize` | 사이즈별 경쟁 비중 맵 (`size` 문자열 → 0~1 ratio). `*Pct` 필드처럼 0~100 percent로 해석하지 않는다 |
...
2차 드로워가 열릴 때 live 기준 AI 코멘트를 요청한다. 이 요청은 후보 아이템 스냅샷을 자동 저장하거나 수정하지 않는다. 사용자가 2차 드로워에서 저장/수정 버튼을 눌렀을 때만 반환된 `llmAnswer`가 후보 아이템 `details.drawer2.aiComment.answer`에 저장된다.
...
| 필드 | 의미 |
|------|------|
| `llmPrompt` | 백엔드가 사용한 프롬프트/요약 컨텍스트. 스냅샷 저장 시 `drawer2.aiComment.prompt`로 보관한다 |
| `llmAnswer` | 화면의 AI 코멘트 카드에 표시할 본문. 스냅샷 저장 시 `drawer2.aiComment.answer`로 보관한다 |
| `generatedAt` | 생성 시각 |
...
| `details` | 저장 시점의 **`OrderSnapshotDocumentV2` 전체 JSON**. 스냅샷 없이 후보군에 담긴 아이템은 `null`이다 |
...
- `AppendCandidateItemPayload`: `{ companyUuid, stashUuid, skuGroupKey, details, isLatestLlmComment }` — `details`가 오더 스냅샷 저장의 단일 경로이며, `isLatestLlmComment`는 호출부가 현재 AI 코멘트와 스냅샷 기준의 일치 여부를 명시한다.
- `AppendCandidateItemResponse`: 없음 — 단건 append는 `Promise<void>`/empty response 계약이다. 백엔드는 저장 성공을 item summary로 표현하지 않고, 프론트는 성공 직후 응답 객체를 로컬 목록에 삽입하지 않는다. 신규 item summary가 필요한 추천 적용 흐름은 bulk `AppendCandidateItemsResponse`만 사용한다.
...
- `updatedItem.details`는 반드시 아래 5.5절의 `OrderSnapshotDocumentV2` v2 스키마와 동일한 JSON이어야 한다. 백엔드는 `schemaVersion: 2`, `skuGroupKey`, `savedAt`, `context`, `drawer1`, `drawer2`를 채워 `CANDIDATE_ITEM.details`에 저장한다.
...
- `drawer2.confirmedTotals`는 현재 스냅샷의 `drawer2.sizeOrders[].confirmQty` 기준 총합이다. 상세확정 저장 후에는 확정 오더 총합으로 사용되며, 스냅샷 기준 보기, 엑셀/오더 근거 복원에 필요하므로 누락하면 안 된다. AI 코멘트를 함께 생성하는 경우 `drawer2.aiComment.prompt`, `drawer2.aiComment.answer`도 저장한다.
...
| `safetyStockCalc` | 안전재고 기준 결과 블록 |
| `forecastQtyCalc` | 포캐스트 수량 모드 결과 블록 — **`safetyStock`은 `null` 고정 타입**(수식 분기용) |

---

### 3.x `getSecondaryAiComment(params)` 요청 보강

`getSecondaryAiComment`는 저장 mutation이 아닌 read-like POST다. `skuGroupKey`는 path parameter로 보내고, 나머지 값은 JSON body로 보낸다.

| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `periodStart` | string | Y | AI 분석 시작일 |
| `periodEnd` | string | Y | AI 분석 종료일 |
| `forecastMonths` | number | Y | 예측 개월 수 |
| `competitorChannelId` | string | Y | 비교 기준 경쟁 채널 ID |
| `companyUuid` | string | N | 선택 회사 scope. `전체` 선택 시 프론트 HTTP adapter가 생략하며 백엔드는 전체 회사 기준으로 처리한다. |
| `candidateItemUuid` | string \| null | N | 후보 item 기준 코멘트 재생성 요청일 때 사용한다. |
| `snapshotForAiComment` | `OrderSnapshotDocumentV2` | N | 클릭 시점의 2차 드로워 스냅샷. 있으면 백엔드는 현재 원천 데이터를 재계산해 덮어쓰지 말고 이 스냅샷을 AI 입력 기준으로 사용한다. |

응답은 `SecondaryAiCommentResult`이며 `llmPrompt`, `llmAnswer`, `generatedAt`을 반환한다.
...
### 4.2 `SecondaryStockOrderRequest`

2차 재고·발주 계산 요청과 snapshot `drawer2.stockOrderRequest`에서 공유하는 입력 계약이다. 화면에 없는 안전재고 모드, 수동 안전재고, 서비스 레벨 값은 프론트 요청/스냅샷에 저장하지 않는다.

| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `currentOrderInboundDueDate` | string | Y | 금번 주문할 물건이 들어올 예정일. 날짜 문자열이다. |
| `nextOrderInboundDueDate` | string | Y | 다음번에 주문할 물건이 들어올 것으로 예상되는 예정일. 날짜 문자열이다. |
| `leadTimeDays` | number | Y | 두 입고 예정일 기준으로 계산된 리드타임 일수. |
| `dailyMeanOverride` | number | N | 사용자가 보정한 일평균 판매량. 없으면 백엔드 계산값을 사용한다. |

### 4.2.1 `SecondaryStockOrderResult`

2차 재고·발주 계산 응답과 snapshot `drawer2.stockOrderResult`에서 공유하는 결과 계약이다.

| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `trendDailyMean` | number | Y | 일별 추이 기준 기본 일평균 판매량. |
| `dailyMean` | number | Y | 오더 계산에 실제 적용한 일평균 판매량. |
| `sigma` | number | Y | 안전재고 공식에 쓰는 표준편차. 공식과 표본/모집단 기준은 백엔드 구현 전 사용자 확인이 필요하다. |
| `display` | object | Y | 사이즈별 재고, 미입고 잔량, 입고예정 잔량 표시값. |
| `safetyStockCalc` | object | Y | 안전재고 기준 추천 수량과 금액 결과. |
| `forecastQtyCalc` | object | Y | 예측 수량 기준 추천 수량과 금액 결과. `safetyStock`은 `null`이다. |
...
### 5.5 `OrderSnapshotDocumentV2` (v2 스키마)
...
| `skuGroupKey` | 상품 단위 묶음 키. `SKU.code + SKU.color_code`에 대응하며 DB `SKU.uuid`가 아니다 |
| `companyUuid` | 회사 scope. 새 단일 회사 스냅샷에는 포함하는 것이 원칙이고, 누락은 명시적 unscoped 스냅샷으로 본다 |
...
| 필드 | 의미 |
|------|------|
| `competitorBasis` | current v2 스냅샷의 2차 드로워 판매 기준 데이터. `ProductSecondaryDetail` 전체가 아니라 `skuGroupKey`, `competitorPrice`, `competitorQty`, `competitorRatioBySize`만 저장한다. `competitorRatioBySize`는 0~1 ratio이며 percent가 아니다 |
| `competitorChannelId`, `competitorChannelLabel` | 선택 경쟁 채널 |
| `stockOrderRequest` | [`SecondaryStockOrderRequest`](#42-secondarystockorderrequest). current v2 parser는 금번/차기 입고 예정일, 리드타임, 선택 일평균 보정값만 저장한다 |
| `stockOrderResult` | [`SecondaryStockOrderResult`](#421-secondarystockorderresult). 저장 당시 계산 응답. 계산 API 응답이 없으면 생략한다 |
| `unitEconomics` | 저장 당시 통합 오더 설정의 단가·원가·예상 수수료율: `{ unitPrice, unitCost, expectedFeeRatePct }`. `expectedFeeRatePct`는 0~100 percent다. 스냅샷 기준 보기에서 live 판매 정보로 재계산하지 않고 이 값을 표시한다 |
| `selfWeightPct` | 자사 가중치. 0~100 percent |
| `bufferStock` | 추가 버퍼 재고 |
| `aiComment` | AI 코멘트 컨텍스트: `{ prompt, answer }` |
| `confirmedTotals` | 현재 스냅샷의 `sizeOrders[].confirmQty` 기준 필수 총합: `orderQty`, `expectedSalesAmount`, `expectedOpProfit`, `expectedOpProfitRatePct`. AI 코멘트 요청용 스냅샷에서는 현재 화면 입력값 합계이고, 저장 후에는 상세확정 오더 총합으로 사용한다 |
| `sizeOrders` | [`OrderSnapshotSizeOrderV2[]`](#56-ordersnapshotsizeorderv2) |



**`drawer2.stockOrderRequest` current v2 fields**

`drawer2.stockOrderRequest`는 화면 입력 기준으로 `currentOrderInboundDueDate`, `nextOrderInboundDueDate`, `leadTimeDays`를 required로 취급한다. `dailyMeanOverride`는 사용자가 일평균을 보정한 경우에만 포함한다. 계산 결과인 `trendDailyMean`, `dailyMean`, `sigma`, 재고 표시값, 추천 수량/금액은 `drawer2.stockOrderResult`에 저장한다.

`currentOrderInboundDueDate`는 금번 주문할 물건이 들어올 예정일이고, `nextOrderInboundDueDate`는 다음번에 주문할 물건이 들어올 것으로 예상되는 예정일이다. 화면에 없는 `safetyStockMode`, 수동 안전재고, 서비스 레벨 값은 프론트 요청/스냅샷에 저장하지 않는다.

### 5.6 `OrderSnapshotSizeOrderV2`

| 필드 | 의미 |
|------|------|
| `size` | 사이즈 |
| `selfSharePct`, `competitorSharePct`, `blendedSharePct` | 자사/경쟁/블렌드 점유율. 0~100 percent |
| `forecastQty` | 예측 수량 |
...
- 단일 회사 선택 시 프론트는 분석, 산점도, filter meta, 후보군, 상품 드로워, 2차 드로워, read-like POST 오더 계산 요청에 `companyUuid=<CompanySummary.uuid>`를 포함한다.
- `전체` 선택 시 프론트는 `companyUuid`를 생략하고, 백엔드는 해당 업무 조회에서 회사 `WHERE` 조건을 적용하지 않는다.
- 2차 AI 코멘트와 2차 재고·발주 계산은 POST 요청이지만 저장/수정/삭제/job 시작이 아니므로 read-like optional scope로 처리한다. `ALL_COMPANY_UUID`, 빈 문자열, 누락 scope는 HTTP body에서 `companyUuid` 생략으로 정규화된다.
- 후보군 mutation, 후보군 관련 job 시작, 그리고 해당 job/SSE subscribe는 side effect가 있는 단일 회사 업무 흐름이다. 이 계열은 `companyUuid`가 필수이며 `전체`, 빈 문자열, 누락 scope를 검증 오류로 처리한다.
...
| 2차 AI 코멘트 | `POST /products/:skuGroupKey/secondary/ai-comment` | body `companyUuid?` | 생략 시 전체 회사 기준. `snapshotForAiComment?`가 있으면 현재 2차 드로워 스냅샷 기준으로 생성 |
...
## 10. 2026-05-22 TODO-076 contract alignment notes

### 10.1 ApiFailureKind literal

- Frontend `ApiFailureKind` uses `auth` for HTTP 401.
- Do not document or implement `authentication` as an API/client failure literal.
- 401 remains authentication/session failure, but the normalized client kind is exactly `auth`.
- 403 remains `permission`; auth and permission failures must stay distinct in UX and backend error handling.

### 10.2 Company scope required for candidate/job/SSE/order metric flows

- Candidate stash mutation, candidate item mutation, candidate import, candidate detail confirmation job start, candidate LLM comment job start, and their SSE subscribe requests require a single company scope.
- Candidate order metric SSE also requires `companyUuid`. The adapter calls the required mutation-scope helper before opening the event stream; omitted, empty, or all-company scope is not a valid backend contract for this stream.
- Backend should validate missing or invalid `companyUuid` as request validation failure with an `ApiErrorResponse` message that identifies `companyUuid`. Prefer HTTP 422 when the frontend should classify the response as `validation`; HTTP 400 is not normalized to `validation` by the current frontend policy and falls through to `client` unless the frontend mapping changes.
- If the authenticated user cannot access the requested company or job, return 403. If the job ID does not exist in the visible scope, return 404.
- Read-like dashboard queries may still omit `companyUuid` for all-company reads where documented. That exception does not apply to side-effect flows or order metric SSE.

### 10.3 Mutation success and refresh failure separation

- A successful mutation response is the authoritative result of the write. A later refresh/read failure must not turn the already committed mutation into a failed mutation.
- UI should keep the mutation result or previous stable data visible and surface the refresh failure separately as stale UX.
- Backend should avoid fake success states. If the write fails, return an error. If the write succeeds but a follow-up read fails, the follow-up read should fail independently.

### 10.4 Partial success and pending failure policy

- Bulk operations may report partial success only when successful and failed item counts are distinguishable.
- Partial success must include enough detail for UI to reflect successful rows and failed rows separately.
- SSE completion with missing item results is not full success. Remaining pending items should be treated as failed or not-calculated by the UI, and backend events should make the terminal state explicit where possible.

## 2026-05-26 snapshot contract alignment

- Current persisted snapshot document type is `OrderSnapshotDocumentV2` with `schemaVersion: 2`.
- Current v2 top-level fields are `schemaVersion`, `skuGroupKey`, optional `companyUuid?: string`, `savedAt`, `context`, `drawer1`, and `drawer2`.
- In snapshot fields, `competitorRatioBySize` is a 0~1 ratio. Share, weight, and fee `*Pct` fields are 0~100 percent. `expectedOpProfitRatePct` is an operating-profit percent point and may be negative when expected operating profit is negative.
- `confirmedTotals` is a required aggregate of the current snapshot `sizeOrders[].confirmQty`. After detail confirmation is saved, the same object is used as the saved confirmed order total.
- Candidate item create/update payloads store the order snapshot in `details`; do not document or accept `snapshot` as the candidate item payload field name.
- Frontend snapshot parsing preserves finite negative business values so saved state can be surfaced. Backend write validation remains responsible for rejecting negative quantities or amounts where the business domain does not allow them.
- Single-company hydrate is valid only when `snapshot.companyUuid === selectedCompanyUuid`.
- All-company hydrate is valid only for unscoped snapshots where `snapshot.companyUuid` is missing.
- If the scope does not match, reload live scoped data or expose an unavailable snapshot state. Do not fabricate fallback business values.
- Candidate mutation, job, import, and SSE flows still require a concrete company scope even though documented read APIs may omit `companyUuid`.

## 2026-05-27 candidate append and drawer scope contract alignment

### Singular candidate append item payload

- Singular candidate item append is separate from bulk append.
- Singular append payloads must include `details: OrderSnapshotDocumentV2`.
- Singular append payloads must include `isLatestLlmComment: boolean`.
- `details` is the persisted candidate item snapshot field name. Do not document or accept `snapshot` as a singular append payload alias.
- `isLatestLlmComment` records whether the persisted AI comment belongs to the latest drawer/snapshot basis at append time.

### Bulk candidate append payload

- Bulk append remains a different contract from singular append.
- Bulk append may use `skuGroupKeys` and batch result fields instead of a per-item `details` payload.
- Do not infer singular append required fields from the bulk append payload shape, and do not make singular append accept a bulk-only shape.

### Candidate detail drawer company scope

- Candidate detail drawer flows must preserve explicit company scope across item detail, primary bundle, secondary detail, and secondary mutation calls.
- Candidate item detail read may keep read-like optional scope only where the catalog explicitly marks it optional, but the detail drawer must pass the selected `companyUuid` when rendering a single-company candidate item.
- `getProductDrawerBundle` used by the candidate detail drawer must receive the same explicit `companyUuid` as the selected candidate/company scope.
- `getProductSecondaryDetail` used by the candidate detail drawer must receive the same explicit `companyUuid`.
- Secondary mutations from the candidate detail drawer, including candidate item update/confirm flows, require the same concrete single-company `companyUuid`; omitted, empty, or all-company scope is invalid.

...
- 관리자 API 키 관리는 `src/api/types/admin.ts` 계약과 `src/api/mock/adminApi.ts` mock을 통해서만 접근한다. 화면은 키 원문을 입력/교체 요청에만 담고, 목록 응답은 `maskedKey`만 표시한다. 현재 mock은 DB 대체 저장소가 아니라 런타임 메모리에서 마스킹 값과 메타데이터만 보관한다.
...
| `api/types/*` | 프론트-백엔드 계약 타입. 관리자 API 키 계약은 `admin.ts`, 인증 계약은 `auth.ts`, 후보군 계약은 `candidate.ts`, 저장 스냅샷 계약은 `snapshot.ts`, 2차 드로워 계약은 `secondary.ts`가 소유한다. | 요청/응답 구조가 바뀌면 먼저 수정 |
...
| 파일 | 역할 |
|------|------|
| `admin.ts` | 관리자 API 키 목록/추가/메타 변경/키 교체/연결 테스트 계약. 요청 payload에는 `plainKey`가 있지만 목록 응답은 `maskedKey`만 포함한다 |
...
| 파일 | 역할 |
|------|------|
| `adminApi.ts` | mock 관리자 API 키 관리 구현. 관리자 세션만 허용하고, 키 원문은 응답/목록 상태에 남기지 않는다 |
...
| 파일 | 역할 |
|------|------|
| `AdminPage.tsx` | 관리자 라우트 shell. 공통 헤더 안에 사용자 관리/API 키 관리 탭과 현재 탭 설명을 배치한다 |
...
| `adminHelpers.ts` | 관리자 화면의 역할/키 공급자/용도 option, 테스트 상태 label, 공통 날짜/오류 표시 helper |
...
| `candidateOrderExcelExport.ts` | 이미 조회한 후보군 아이템의 `orderExport` DTO를 발주용 XLSX로 변환하고 다운로드 파일명을 만든다. 데이터 행 생성 함수와 시트 스타일 적용 함수를 분리해 두며, `exceljs`는 후보군 상세 목록 로딩 후 미리 로드하고 다운로드 시 같은 promise를 재사용한다. 주 데이터/메타 시트 헤더와 `N/A` 셀 스타일을 적용한다 |

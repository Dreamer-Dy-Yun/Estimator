...
- 후보군 상세 필터 카드에는 발주 엑셀 다운로드 액션을 둔다. 현재 프론트는 후보군 아이템 `details` 스냅샷에서 제품 1개당 1행의 발주 데이터를 생성한다. 주 데이터 시트는 기본 식별/배지/오더/판매 지표 컬럼 뒤에 후보군 전체 사이즈를 동적 컬럼으로 붙이고, 제품에 없는 사이즈는 `-`로 표시한다. 복수 배지는 한 셀 안에서 줄바꿈한다. 메타 시트에는 오더 입고 예정일과 사용자 이름을 둔다. 추후 백엔드 다운로드 endpoint로 이전할 수 있도록 XLSX 생성은 `candidate-stash` feature와 `utils/xlsxWorkbook.ts`로 분리한다.

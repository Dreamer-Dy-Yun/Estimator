...
| 영역 | 소유 문서 | 책임 |
|------|-----------|------|
| product drawer snapshot boundary | [product-drawer.md](./boundaries/product-drawer.md) | current v2 스냅샷은 `drawer2.competitorSalesBasis`를 2차 판매 기준 필드로 사용한다. `drawer2.secondary`는 새 스냅샷 필드가 아니다. |
...
| backend snapshot contract | [../backend-api/backend-api-spec.md](../backend-api/backend-api-spec.md), [../backend-api/dashboard-api-contract-catalog.md](../backend-api/dashboard-api-contract-catalog.md) | 백엔드 구현자가 `drawer1.summary`, `drawer2.competitorSalesBasis`, 계산 입력/결과, 확정 row의 의미와 저장 용도를 이해할 수 있게 필드 설명을 유지한다. |
| legacy snapshot parser normalize | `dashboard-app/src/snapshot/parseOrderSnapshot.ts` | 기존 v2 legacy JSON의 `drawer2.secondary`는 parser normalize 단계에서 `drawer2.competitorSalesBasis`로 흡수한다. 새 문서와 새 payload는 current field를 기준으로 한다. |

`drawer1.summary`는 compact summary다. 상품 식별/표시 메타와 기간 KPI를 저장하며 `sizeMix`, `seasonality`, `recommendedOrderQty`, `monthlySalesTrend`를 저장하지 않는다. compact snapshot hydrate는 빈 배열, `0`, 임의 계산값으로 비저장 필드를 보정하지 않는다.

## TODO-116 snapshot v2 company scope source boundary

| Area | Owner document | Responsibility |
|------|----------------|----------------|
| Current snapshot type name | Product drawer boundary, backend API spec, API catalog | Use `OrderSnapshotDocumentV2` for current `schemaVersion: 2` snapshots. Use `OrderSnapshotDocumentV1` only as deprecated compatibility wording. |
...
| Compact drawer2 contract | Product drawer boundary, backend API catalog | Current payloads use `drawer2.competitorSalesBasis`. `drawer2.secondary` is legacy parser input only. |
| Regression coverage | `dashboard-app/src/snapshot/parseOrderSnapshot.test.ts` | Lock current compact key behavior, legacy normalization, required `stockInputs`, and optional `stockDisplay` handling. |

...
| `useCandidateBulkDetailConfirm.ts` | 상세 일괄확정 시작, SSE subscription, `bulkConfirmBusy`, `bulkConfirmProgress`, progress popup close timer, 완료/실패 toast를 맡는다. | `mountedRef`로 unmount 이후 반영을 막고, `sequenceRef`로 job 단위 async stale guard를 유지한다. 이전 상세확정 작업의 SSE 이벤트, 에러 callback, close timer가 최신 `bulkConfirmProgress`를 닫거나 덮지 못해야 한다. 기간 변경과 item load seq 충돌 정책은 caller 계약과 함께 다루되, hook 내부 job sequence guard는 이 파일의 책임이다. | 일괄확정 progress UX, subscription 종료, `updatedItem` 반영 callback 호출, job sequence guard 경계를 바꿀 때만 수정한다. 스냅샷 저장/해제 계산, 기본 리스트 재조회, 추천 state 책임을 합치지 않는다. |

...
- 현재 v4 산식은 n차 제안 계산 시 `[n차 입고일, n+1차 입고일)` 구간의 수요와 같은 구간의 `expectation`을 날짜순 재고 흐름으로 처리한다. 기존 주문 입고예정량은 실제 입고일에 더하고, 일별 판매예측을 차감한 뒤 최저 예상 재고가 UI 재고 하한보다 낮아지는 만큼을 제안한다.

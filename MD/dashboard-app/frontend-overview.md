...
- 현재 목 구현은 [`api/mock/authApi.ts`](../../dashboard-app/src/api/mock/authApi.ts)의 정적 사용자 seed로 로그인 ID와 비밀번호를 확인하고, 세션은 런타임 메모리에만 둡니다. 헤더 우상단의 사용자 정보 버튼은 로그인 ID와 역할을 표시하고, 모달에서 로그인 ID와 비밀번호 변경 API를 호출합니다.

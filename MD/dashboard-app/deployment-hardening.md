...
- Pages API mode: `VITE_USE_MOCK_API=false`
- Pages API base URL: GitHub Actions variable `DASHBOARD_API_BASE_URL` → `VITE_API_BASE_URL`
...
배포 workflow는 `VITE_USE_MOCK_API=false`와 `VITE_API_BASE_URL=${DASHBOARD_API_BASE_URL}`을 명시해 HTTP adapter 번들을 생성한다. 로컬 개발에서 mock가 필요하면 `VITE_USE_MOCK_API=true`를 명시한다. 환경변수 미설정 상태는 HTTP mode로 동작해야 하며, mock mode가 기본값이 되어서는 안 된다.

`DASHBOARD_API_BASE_URL`은 secret이 아니라 브라우저 번들에 노출되는 API origin이므로 GitHub Actions variable로 관리한다. 값이 없으면 workflow의 runtime configuration step에서 빌드 전에 실패시킨다.
...
| 운영 배포가 mock API로 빌드됨 | `VITE_USE_MOCK_API` 누락 또는 기본값 의존 | workflow에서 `VITE_USE_MOCK_API=false`를 명시하고 앱 기본값도 HTTP로 둔다 | mock 배포가 필요할 때만 별도 환경에서 `true`를 명시한다. |
| HTTP 배포본이 사용자 브라우저의 localhost를 호출함 | `VITE_API_BASE_URL` 누락 | workflow에서 `DASHBOARD_API_BASE_URL` preflight 후 `VITE_API_BASE_URL`로 주입 | production HTTP mode에서 backend base URL 누락을 허용하지 않는다. |
...
- 운영/Pages 배포에서 `VITE_USE_MOCK_API` 누락을 mock 사용으로 해석하지 않는다.
- 운영/Pages HTTP 배포에서 `VITE_API_BASE_URL` 누락을 localhost fallback으로 처리하지 않는다.

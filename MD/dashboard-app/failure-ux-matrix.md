...
- stale UX는 최신 요청이 실패했을 때 이전 화면 데이터를 유지하면서 현재 실패를 표시하는 사용자 경험이다.
- async stale guard는 늦게 도착한 이전 요청 응답이나 SSE 이벤트가 최신 상태를 덮지 못하게 무시하는 내부 방어다. 이 guard 자체는 사용자에게 표시할 실패 surface가 아니다.

## stale UX와 async stale guard 구분

| 구분 | 기준 | 화면 처리 | 금지 |
|------|------|-----------|------|
| stale UX | 최신 요청이 실패했지만 이전 렌더링 데이터가 남아 있음 | 기존 데이터는 유지하고, 최신 요청 실패를 기존 error surface, toast, row/cell 실패 배지로 함께 표시 | 실패를 빈 목록이나 성공 상태로 대체하기 |
| async stale guard | 이전 요청, 이전 기간, 이전 SSE job의 응답이 최신 요청보다 늦게 도착함 | 응답을 조용히 무시하고 최신 화면 상태를 유지한다. 별도 실패 UX를 만들지 않는다. | 늦은 응답으로 최신 items, 추천 배지, 오더 지표, 상세확정 상태를 되돌리기 |
| 둘 다 필요한 경우 | 최신 요청 실패와 이전 데이터 유지가 동시에 발생함 | 이전 데이터 유지 여부는 stale UX로 표시하고, 더 오래된 응답은 async stale guard로 계속 무시한다. | stale UX를 이유로 과거 응답을 다시 적용하기 |
...
| 이너 후보군 기본 리스트 | 목록 조회 실패 | 기존 이너 리스트 error surface. 최신 요청 실패 시 stale rows가 있으면 유지하고 최신화 실패 메시지 표시 | 리스트를 비우거나 새 실패 카드를 삽입해 레이아웃 밀기 |
...
| fallback | 허용 조건 |
|----------|-----------|
| stale 데이터 유지 | 최신 요청 실패를 함께 표시하고, 사용자가 이전 데이터임을 알 수 있을 때. 늦은 이전 응답을 적용하는 async stale guard 예외가 아니다. |

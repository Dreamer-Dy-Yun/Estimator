...
| 회사 scope | `getCompanies` 응답은 `uuid`, `name`만 필요하다. 회사 소유 업무 API는 단일 회사 선택 시 `companyUuid`를 포함하고, `전체` 선택 시 생략한다. 분석, 산점도, filter meta, 후보군, 상품 드로워, 2차 드로워, 오더 계산, SSE, mutation 모두 같은 scope 계약을 따른다. |

...
- `src/App.tsx` GitHub Pages hash routing 처리
...
- GitHub Pages의 `/Estimator/` base path는 Vite build base로 맞추고, 앱 내부 경로는 `HashRouter`로 서버 fallback 의존을 줄인다.
...
2. GitHub Pages 정적 배포에서 라우트 새로고침이 깨지지 않도록 `HashRouter`를 적용한다.
...
- 초안의 `BrowserRouter basename` 방식은 Pages/preview 루트에서 빈 화면을 만들 수 있어 `HashRouter`로 바꿨다.

...
- `getSecondaryInboundSplitSource` query: `productSkuGroupKey`, `productUuid?`, `productBrand`, `productCode`, `productColorCode`, `calculationBaseDate`, `coverageStartDate`, `coverageEndDate`, base subject.
...
- Snapshot v7 still stores `ignoreExistingOrderInbound` on each `confirmed.rounds[]` row. The current UI exposes one schedule-level toggle and writes the same value to every row on apply.

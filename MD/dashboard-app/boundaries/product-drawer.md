...
`secondaryDrawer.module.css`가 CSS `@import`로 2차 드로워 카드/컨트롤/표/입력 스타일 조각을 묶는다. 카드 단위 UI는 `cards/*`, hook 경계는 `hooks/*`, 계산 모델은 `model/*`에 둔다.
2차 드로워 상단 메타/액션 행은 3열 grid를 사용하며, 상품 메타 영역은 1~2열, 후보군 정보와 버튼 영역은 3열을 차지한다. 이너 후보 액션 버튼은 동일 폭으로 정렬한다.

...
| Candidate snapshot | 후보 항목에 저장되는 `OrderSnapshotDocument` v8를 생성/파싱한다. |
...
| Split inbound planning source | `getSecondaryStockOrderCalc().inboundSplitSource` | Stock-order-calc response field containing calculation base date, coverage dates, size/date sales forecasts, and supply points. |
...
- Round-level suggested quantities are derived from `stockOrderCalc.inboundSplitSource` interval sales forecasts minus carried stock and existing-order inbound supply.

## 6. Split inbound planning source

`inboundSplitSource` is returned inside `getSecondaryStockOrderCalc`; it is not a separate endpoint. Split count, split dates, confirmed rows, and `ignoreExistingOrderInbound` are UI/snapshot state.
...
이 API는 사용자가 적용한 분할 rows를 저장하거나 반환하지 않는다. 저장된 분할 결과와 `ignoreExistingOrderInbound` 옵션은 후보 스냅샷의 `drawer2.confirmed.rounds`가 소유한다. 다만 UI에서 `ignoreExistingOrderInbound`는 분할 다이얼로그 단일 토글로 관리되며 적용 시 모든 row에 동일하게 반영된다.

## 7. 스냅샷 경계

현재 저장 스냅샷은 `OrderSnapshotDocument` v8이다.

| 영역 | 저장 의미 |
|---|---|
| `drawer1.summary` | 상품 기본 정보와 가용 재고 |
| `drawer2.stockOrderRequest` | 오더 계산에 사용한 입고일/오더 커버리지 일수/수요 override |
| `drawer2.stockOrderResult` | 계산 결과. 필수 필드 |
...
## Current stock-order inboundSplitSource contract
...
- If the global `ignoreExistingOrderInbound` flag is enabled in the split dialog, existing-order inbound supply points inside all split intervals are excluded from the suggestion calculation.
- Snapshot v8 still stores `ignoreExistingOrderInbound` on each `confirmed.rounds[]` row. The current UI exposes one schedule-level toggle and writes the same value to every row on apply.

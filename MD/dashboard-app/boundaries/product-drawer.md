...
| `src/snapshot/parseOrderSnapshot.ts` | Saved snapshot parsing and current contract validation |
...
| `drawer2.stockOrderResult` | 재고·발주 계산 응답. 일평균, 표준편차, 안전재고 계산 결과, 예측수량 계산 결과, 표시용 재고/잔량을 저장한다. |
| `drawer2.unitEconomics` | 저장 당시 단가, 원가, 예상 수수료율. |
...
| 상황 | 처리 |
|------|------|
| live bundle ?? + parsed snapshot | fallback? ???? ???. current compact snapshot??? full `ProductPrimarySummary`? ??? ???. |
| live bundle ?? + parsed snapshot | fallback? ???? ???. current compact snapshot??? full `ProductPrimarySummary`? ??? ???. |
| live bundle missing + parsed snapshot | No fallback. Parsed current snapshots do not contain enough data to create a full `ProductPrimarySummary`. |

### current validation

Frontend `parseOrderSnapshot` requires `drawer2.competitorBasis`; downstream logic uses current fields only.

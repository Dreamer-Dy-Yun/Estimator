...
`src/api/mock`에는 드로어 수량/금액 계산을 눈으로 검산하기 위한 `TEST-TOP` / `테스트 상의` SKU가 있다. 이 SKU는 기본 후보군 A에 seed로 들어가며, 자사/경쟁사 분석 목록과 이너후보군 2차 드로어에서 같은 `skuGroupKey`로 조회된다.

단일 회사 scope(`한아INT`, `T1글로벌`)에서 보는 검증 기준값:
...
| 경쟁 평균 판매가 | 110,000원 |
| 경쟁 기간 판매량 | 2,400EA |
| 경쟁 월간 판매량 | 크림 기준 매월 200EA |
| 사이즈 | S, M, L, XL, XXL |
| 사이즈별 자사/경쟁 비중 | 각 20% |
| 사이즈별 자사 판매량 | 각 240EA |
| 사이즈별 기본 확정 수량 | 각 200EA, 합계 1,000EA |
| 2차 드로어 기본 가용 재고 | 600EA, 사이즈별 120EA |
| 2차 드로어 표시용 잔량 | 총 오더 잔량 200EA(각 40EA), 미입고 잔량 100EA(각 20EA) |

이 값은 운영 계약이 아니라 mock 계산 검증용 fixture다. 실제 백엔드는 이 값을 특별 취급하지 않고 `ProductPrimarySummary`, `ProductSecondaryDetail`, 후보군 item/snapshot 계약만 맞춘다.

mock raw catalog는 전체 회사 합산 기준으로 위 값의 2배를 가진다. `TEST-TOP`은 mock 회사 scope에서 `한아INT`와 `T1글로벌`이 각각 50%를 갖도록 고정해, 단일 회사 후보군/드로어 검증 시 위 표의 값이 그대로 표시되게 한다.

...
| live bundle 없음 + parsed snapshot | parser 통과 후에는 legacy full summary 필드가 제거되므로 parsed `OrderSnapshotDocumentV2`만으로 legacy full summary fallback을 수행하지 않는다. |
...
- Current product drawer snapshot type is `OrderSnapshotDocumentV2`; `OrderSnapshotDocumentV1` is deprecated compatibility wording only.
...
- New payloads use `drawer2.competitorSalesBasis`; `drawer2.secondary` remains legacy parser input only.

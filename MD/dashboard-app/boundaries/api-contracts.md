...
| `mock/mockProductComparisonApi.ts` | 상품 비교 target, 월간 추이, 판매 정보, 2차 일간 추이 mock |
...
- `mockProductComparisonApi.ts` implements the same source contract for mock mode. Mock source generation is a mock backend substitute, not a UI fallback.

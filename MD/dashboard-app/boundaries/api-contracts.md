...
| `mock/secondaryInboundSplitSourceFixtures.json` | 분할 입고 source fixture. Mock mode 배포 산출물에 포함하며 정적 호스팅 HTTP gzip/brotli 압축을 전제로 둔다 |
...
- Daily trend source is aggregate per date. It uses symmetric `base`/`comparison` subject flow cells with `{ sale, inbound }`, and `inbound` is nullable for unavailable comparison inbound.
...
- `mockProductComparisonApi.ts` implements the same source contract for mock mode. `secondaryInboundSplitSourceFixtures.json` is the mock backend substitute data source for split inbound tests and Pages preview; it stays under `src/api/mock` and is served as a compressed static build asset by the deployment host.

...
- 헤더 회사 선택은 분석 API의 전역 company scope다. 단일 회사 선택 시 `companyUuid`를 요청에 포함하고, `전체` 선택 시 생략해 전체 회사 기준으로 조회한다.
...
`AnalysisCandidateBulkAddModal.tsx`가 분석 리스트에서 선택한 상품을 기존 후보군에 넣거나 새 후보군을 만든 뒤 넣는 흐름을 소유한다. 스냅샷은 만들지 않고 `appendCandidateItems`만 호출한다. 회사 선택이 `전체`이면 버튼은 숨기지 않고 비활성화해, 후보군은 단일 회사 기준으로만 추가된다는 제한을 드러낸다.

...
자사/경쟁사 분석 탭은 후보군에 상품을 담는 입구다. 분석 리스트의 체크박스와 `선택한 물품을 후보군으로` 모달은 스냅샷 없이 `stashUuid + skuGroupKeys`만 API에 전달한다. 단, 헤더 회사 선택이 `전체`이면 후보군 추가 액션은 비활성화한다.
...
- 헤더 회사 선택은 회사 소유 업무 데이터의 전역 company scope다. 분석 목록, 산점도, filter meta, 분석 페이지에서 여는 상품 드로워 요청은 단일 회사 선택 시 `companyUuid`를 포함하고, `전체` 선택 시 생략해 전체 회사 기준으로 조회한다.
- 화면 표시에서 기존 `자사` 비교 축은 헤더에서 선택한 회사명으로 표시한다. `전체` 선택 시에는 `자사 전체`로 표시한다. 단, 상단 `자사 분석` 탭명과 경쟁사 분석 리스트 column header는 고정 문구를 유지한다.
...
`AnalysisCandidateBulkAddModal.tsx`가 분석 리스트에서 선택한 상품을 기존 후보군에 넣거나 새 후보군을 만든 뒤 넣는 흐름을 소유한다. 스냅샷은 만들지 않고 `appendCandidateItems`만 호출한다. 단일 회사 선택 시 후보군 추가 payload에는 `companyUuid`가 포함된다. 회사 선택이 `전체`이면 버튼은 숨기지 않고 비활성화해, 후보군은 단일 회사 기준으로만 추가된다는 제한을 드러낸다.
